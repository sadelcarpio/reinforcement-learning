...
public record EpsilonGreedy(KArmedBanditsModel model, double epsilon) {
    ...
    public ArrayList<Double> run(int steps) {
        ArrayList<Double> q = new ArrayList<>(model.k);
        ...
        for (int i = 0; i < steps; i++) {
            ...
            n += 1;
            q.set(a, q.get(a) + (1. / n) * (reward - q.get(a)));
        }
        return q;
    }

    public static ArrayList<Integer> getArgmax(List<Double> q) {
